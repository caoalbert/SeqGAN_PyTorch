...
MLE_TRAIN_EPOCHS = 10
ADV_TRAIN_EPOCHS = 3
D_STEP_DISCRIMINATOR = 3
POS_NEG_SAMPLES = 34147
...
oracle_samples_path = 'SeqGAN_input.pt'
...
if __name__ == '__main__':
    ...
    if CUDA:
        oracle = oracle.cuda(device)
        gen = gen.cuda(device)
        dis = dis.cuda(device)
        oracle_samples = oracle_samples.cuda(device)
    ...
    train_discriminator(dis, dis_optimizer, oracle_samples, gen, oracle, D_STEP_DISCRIMINATOR, 3)
    ...
    torch.save(gen.state_dict(), 'gen_adv_trained.trc')
    generated_samples = gen.sample(20000).cpu().data.numpy()
